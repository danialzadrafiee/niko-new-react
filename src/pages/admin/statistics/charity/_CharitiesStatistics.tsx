...
console.log(statistics)
  ...
  useEffect(() => {
    const filtered = statistics.filter(charity => 
      (charity.charity_name?.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (charity.charity_id?.toString().toLowerCase().includes(searchTerm.toLowerCase()))
    )
      ...
      {isLoading ? (
        ...
        <>
            ...
            <Card className="bg-green-50">
              ...
              <CardContent>
                <p className="text-3xl font-bold text-green-800">
                  {statistics.reduce((sum, charity) => sum + charity.live_fundraises, 0)}
                </p>
              </CardContent>
            </Card>
            <Card className="bg-yellow-50">
              ...
              <CardContent>
                <p className="text-3xl font-bold text-yellow-800">
                  {statistics.reduce((sum, charity) => sum + charity.total_amount_raised, 0).toFixed(2)} تومان
                </p>
              </CardContent>
            </Card>
            <Card className="bg-purple-50">
              ...
              <CardContent>
                <p className="text-3xl font-bold text-purple-800">
                  {(statistics.reduce((sum, charity) => sum + charity.average_completion_percentage, 0) / statistics.length).toFixed(2)}%
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <CharityTable
            ...
            onViewDetails={handleViewCharityDetails}
          />

          <CharityDetailsDialog
            charity={selectedCharity}
            isOpen={isCharityDialogOpen}
            onOpenChange={setIsCharityDialogOpen}
            onViewFundraiseDetails={handleViewFundraiseDetails}
          />

          <FundraiseDetailsDialog
            fundraise={selectedFundraise}
            isOpen={isFundraiseDialogOpen}
            onOpenChange={setIsFundraiseDialogOpen}
          />
        </>
      )}
    </div>
