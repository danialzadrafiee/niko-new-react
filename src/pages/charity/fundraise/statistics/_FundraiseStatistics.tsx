...
const FundraiseStatistics: React.FC = () => {
  ...
  const fetchStatistics = async () => {
    try {
      ...
      showSuccessToast('آمار صندوق‌های خیریه با موفقیت دریافت شد');
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-4" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <StatCard title="سرمایه‌گذاری‌ها" icon="🤝">
          <StatItem label="تعداد کل سرمایه‌گذاران" value={statistics.total_investors} />
          <StatItem label="تعداد کل سرمایه‌گذاری‌ها" value={statistics.total_investments} />
          <StatItem label="میانگین کمک" value={`${statistics.average_donation_amount.toFixed(2)} تومان`} />
        </StatCard>
        <StatCard title="محدوده کمک‌ها" icon="📈">
          <StatItem label="بزرگترین کمک" value={`${statistics.largest_single_donation.toFixed(2)} تومان`} />
          <StatItem label="کوچکترین کمک" value={`${statistics.smallest_single_donation.toFixed(2)} تومان`} />
        </StatCard>
      </div>
    </div>
  );
