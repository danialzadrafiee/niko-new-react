...
import { ArrowUpDown, ChevronDown, Eye, Trash2, CreditCard, BarChart, MoreHorizontal, Calendar, DollarSign, Users, EyeOff } from "lucide-react"
...
const FundraiseOwned: React.FC = () => {
  ...
  const { fundraises, loading, fetchFundraises, deleteFundraise, updateFundraise } = useFundraiseStore()
  ...
  const handleWithdrawRequest = async (id: number) => {
    await updateFundraise(id, { status: "withdraw_requested" })
  }
  ...
  const renderStatistics = (fundraise: Fundraise) => {
    const totalInvested = fundraise.invests.reduce((sum, invest) => sum + invest.amount, 0)
    const anonymousInvests = fundraise.invests.filter(invest => invest.anonymous).length
    const publicInvests = fundraise.invests.length - anonymousInvests
          ...
          <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow">
              ...
              <p className="text-center text-sm text-blue-600 mt-2 font-medium">
                {fundraise.completion_percentage.toFixed(2)}% تکمیل شده
              </p>
            </div>
            <div className="grid grid-cols-2 gap-6">
              ...
              <StatItem icon={<DollarSign className="h-5 w-5 text-yellow-500" />} label="تعداد خدمت" value={fundraise.amount.toLocaleString()} />
              <StatItem icon={<Users className="h-5 w-5 text-purple-500" />} label="تعداد کل سرمایه‌گذاری‌ها" value={fundraise.invests.length.toLocaleString()} />
              <StatItem icon={<DollarSign className="h-5 w-5 text-red-500" />} label="مجموع سرمایه‌گذاری" value={totalInvested.toLocaleString()} />
              <StatItem icon={<EyeOff className="h-5 w-5 text-gray-500" />} label="سرمایه‌گذاری‌های ناشناس" value={anonymousInvests.toLocaleString()} />
              <StatItem icon={<Eye className="h-5 w-5 text-indigo-500" />} label="سرمایه‌گذاری‌های عمومی" value={publicInvests.toLocaleString()} />
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <PopoverContent className="w-56">
              <div className="grid gap-4">
                ...
                {fundraise.status === "filled" && (
                  <Button variant="outline" size="sm" onClick={() => handleWithdrawRequest(fundraise.id)}>
